...
const doNotChangeOption = { name: 'Do Not Change', value: '' };
...
const severityOptions = ['feature', 'trivial', 'text', 'tweak', 'minor', 'major', 'crash', 'block'].map(
	(v) => ({ name: v, value: v }),
);
...
/* Tri-state boolean (options with '' / 'true' / 'false' string values) so an
 * update operation can leave a boolean field untouched. */
...
function issueFields(forUpdate: boolean): INodeProperties[] {
	const firstOption = forUpdate ? doNotChangeOption : notSetOption;
	...
	return [
		{
			...
			default: '',
			required: !forUpdate,
			description: forUpdate ? 'New summary (leave empty to keep current)' : 'Issue summary',
		},
		{
			...
			: ([
					{
						...
						default: null,
						description:
							'ID of the project to create the issue in. Provide ID or Name - if the instance has a default project, both can be left empty.',
					},
					{
						...
						type: 'string',
						default: '',
						description: 'Name of the project (used when Project ID is empty)',
					},
			  ] as INodeProperties[])),
		{
		...
		},
		{
			...
			options: [
				...
				},
				{
					...
					name: 'priority',
					type: 'options',
					options: [firstOption, ...priorityOptions],
					default: '',
				},
				{
					...
					type: 'options',
					options: [firstOption, ...reproducibilityOptions],
					default: '',
				},
				{
				...
				},
				{
					...
					name: 'resolution',
					type: 'options',
					options: [firstOption, ...resolutionOptions],
					default: '',
				},
				{
					...
					name: 'severity',
					type: 'options',
					options: [firstOption, ...severityOptions],
					default: '',
				},
				{
					displayName: 'Status',
					name: 'status',
					type: 'options',
					options: [firstOption, ...statusOptions],
					default: '',
				},
				{
					displayName: 'Sticky',
					...
					options: forUpdate
						? triStateOptions
						: [notSetOption, { name: 'Yes', value: 'true' }, { name: 'No', value: 'false' }],
					default: '',
				},
				{
					displayName: 'Tags',
				...
				},
				{
					...
					name: 'viewState',
					type: 'options',
					options: [firstOption, ...viewStateOptions],
					default: '',
				},
				{
...
/* Body expression shared by create/update. Every optional field resolves to
 * undefined when not filled in, so it is omitted from the JSON payload.
 * For update operations even summary/description/category are guarded. */
function issueBody(forUpdate: boolean): { [key: string]: string } {
	const guard = (expr: string) => (forUpdate ? `={{ ${expr} || undefined }}` : `={{ ${expr} }}`);

	return {
		summary: guard('$parameter.summary'),
		...
		handler: '={{ $parameter.additionalFields?.handler ? { name: $parameter.additionalFields.handler } : undefined }}',
		reporter:
